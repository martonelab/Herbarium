...
gs_ptm <- read_sheet("https://docs.google.com/spreadsheets/d/1vzVIT5gjQ0yCGwbAyqB4f8pltOkLfWz27TIFTxnm9hk",
                     sheet = 1,skip = 1, col_types = "icccccDcccccccccccDcccccccnnccccc-")
```
...
```{r}
higher <- classification(df_nm$Genus, db= "itis")
...
final <- meta %>% 
  select( Herbarium, LabelQty, `empty$Phylum`, `empty$Class`, Genus, Qualifier, Species, #Species Author
...
colnames(final)[colnames(final)=="empty$Phylum"] <- "Phylum"
colnames(final)[colnames(final)=="empty$Class"] <- "Class"
```
